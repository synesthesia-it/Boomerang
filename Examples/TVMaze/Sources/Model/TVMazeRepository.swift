...
class TVMazeRepository {
    ...
    func episodeDetail(show: Show) -> Observable<ShowDetail> {
        Observable.combineLatest(self.cast(show: show), self.season(show: show)) { castlist, seasonlist in
    ...
    func actorDetail(person: Person) -> Observable<ActorDetail> {
        Observable.combineLatest(self.castDetail(person: person), self.actor(person: person)) {  credits, detail in
